...
-- Enable RLS on the metadata view  
ALTER VIEW public.certificates_metadata SET (security_barrier = true);

-- Grant access to the metadata view
GRANT SELECT ON public.certificates_metadata TO authenticated;

-- Create RLS policy for the metadata view
CREATE POLICY "Users can view certificate metadata for their agents" 
ON public.certificates_metadata 
FOR SELECT 
USING (
  EXISTS (
    SELECT 1 
    FROM agents a 
    WHERE a.id::text = certificates_metadata.agent_id 
    AND a.customer_id = ANY(get_user_customer_ids())
  )
  OR is_admin()
);

-- Add audit logging for certificate access
CREATE OR REPLACE FUNCTION public.log_certificate_access()
RETURNS trigger AS $$
BEGIN
  -- Log access to private key material
  IF TG_OP = 'SELECT' AND OLD.key_pem IS NOT NULL THEN
    INSERT INTO public.audit_logs (
      ...
      '00000000-0000-0000-0000-000000000000', -- System customer for cert operations
      COALESCE(current_user, 'system'),
      'certificate_key_access',
      'certificate:' || OLD.id,
      jsonb_build_object(
