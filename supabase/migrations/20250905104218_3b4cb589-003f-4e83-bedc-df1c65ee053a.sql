-- Convert notification system from per-customer to system-wide (fixed)
...
BEGIN
    -- First, fix any null updated_by values in existing records
    UPDATE email_providers 
    SET updated_by = created_by 
    WHERE updated_by IS NULL AND created_by IS NOT NULL;
    
    UPDATE email_providers 
    SET updated_by = system_customer_id 
    ...
    -- Now migrate existing email_providers to system-wide
    UPDATE email_providers 
    SET customer_id = system_customer_id
    WHERE customer_id != system_customer_id;
    
    -- Now migrate existing channel_providers to system-wide
    UPDATE channel_providers 
    SET customer_id = system_customer_id
    WHERE customer_id != system_customer_id;
