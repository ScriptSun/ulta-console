-- Delete the incorrect record and insert the correct full prompt
DELETE FROM public.system_settings WHERE setting_key = 'ai.systemPrompt';
...
- For chat, text only. For actions, JSON only.',
      ...
      'created_at', now()::text,
      'author', 'system',
      'notes', 'Complete UltaAI system prompt with full functionality',
      'targets', jsonb_build_array('router', 'chat')
    )
  )
);