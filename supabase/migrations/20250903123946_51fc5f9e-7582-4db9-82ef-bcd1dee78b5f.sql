-- Fix the generate_api_key function - remove STABLE as it performs INSERT
...
BEGIN
  ...
  _api_key := 'sk-' || encode(extensions.gen_random_bytes(32), 'hex');
  _key_prefix := substring(_api_key, 1, 12);
  
  -- Hash the API key for storage using extensions.digest
  _key_hash := encode(extensions.digest(_api_key, 'sha256'), 'hex');
