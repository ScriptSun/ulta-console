-- Delete the existing system prompt setting and recreate it with the correct UltaAI prompt
DELETE FROM system_settings WHERE setting_key = 'ai.systemPrompt';

-- Insert the complete UltaAI system prompt
INSERT INTO system_settings (setting_key, setting_value) VALUES 
('ai.systemPrompt', jsonb_build_object(
...
- For chat, text only. For actions, JSON only.',
  'version', 1,
  'published', true,
  'created_at', now()::timestamp::text,
  'author', 'system',
  'targets', '["router", "chat"]'::jsonb,
  'notes', 'Complete UltaAI system prompt with dual-mode responses'
));