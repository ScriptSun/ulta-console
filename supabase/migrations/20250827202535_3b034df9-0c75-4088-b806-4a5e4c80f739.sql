...
-- Drop and recreate RLS policies for command_policies with specific role permissions
DROP POLICY IF EXISTS "Admins can manage all command_policies" ON public.command_policies;
DROP POLICY IF EXISTS "Viewers can read command_policies in tenant" ON public.command_policies;
...
DROP POLICY IF EXISTS "Default deny command_policies" ON public.command_policies;

-- Recreate command_policies RLS policies with role-based permissions
CREATE POLICY "Admins can manage all command_policies" 
ON public.command_policies 
FOR ALL 
USING (is_admin());

CREATE POLICY "Viewers can read command_policies" 
ON public.command_policies 
FOR SELECT 
USING (customer_id = ANY (get_user_customer_ids()) OR is_admin());

CREATE POLICY "Editors can insert command_policies as inactive" 
ON public.command_policies 
FOR INSERT 
WITH CHECK (
...
CREATE POLICY "Editors can update command_policies but not activate" 
ON public.command_policies 
FOR UPDATE 
USING (
  customer_id = ANY (get_user_customer_ids()) AND 
  get_user_role_in_customer(customer_id, 'editor'::app_role) AND
  NOT is_admin()
)
WITH CHECK (
  updated_by = auth.uid() AND
  active = false  -- Editors cannot activate policies
);

CREATE POLICY "Approvers can activate and update command_policies" 
ON public.command_policies 
FOR UPDATE 
USING (
  customer_id = ANY (get_user_customer_ids()) AND 
...
CREATE POLICY "Viewers can read command_confirmations" 
ON public.command_confirmations 
FOR SELECT 
USING (customer_id = ANY (get_user_customer_ids()) OR is_admin());

CREATE POLICY "Users can insert command_confirmations" 
ON public.command_confirmations 
FOR INSERT 
WITH CHECK (
  customer_id = ANY (get_user_customer_ids()) AND
...
CREATE POLICY "Approvers can update command_confirmations" 
ON public.command_confirmations 
FOR UPDATE 
USING (
  customer_id = ANY (get_user_customer_ids()) AND 
