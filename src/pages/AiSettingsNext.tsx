...
export default function AiSettingsNext() {
  const { toast } = useToast();
  const { user } = useAuth();
  // Next.js would use: const router = useRouter();
  ...
  // Next.js would use: const activeTab = searchParams.get('tab') || 'models';
  ...
  const migrateOldSettings = async () => {
    try {
      ...
      if (!oldResult.success && !oldResult.error?.includes('No rows found')) {
