        ...
        <ScrollArea className="h-[600px] w-full">
          <div className="space-y-4">
            {logs.filter(log => log.type === 'router_response').length === 0 ? (
              ...
              <>
                {/* Show System Prompt once at the top */}
                {(() => {
                  const firstLogWithSystemPrompt = logs
                    .filter(log => log.type === 'router_response')
                    .find(log => log.systemPrompt);
                  
                  return firstLogWithSystemPrompt?.systemPrompt ? (
                    <div className="text-sm bg-amber-50 dark:bg-amber-900/20 p-3 rounded border mb-6">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-medium text-amber-800 dark:text-amber-200">
                          📋 System Prompt ({firstLogWithSystemPrompt.systemPrompt.length} chars) - SHARED FOR ALL REQUESTS
                        </span>
                        <div className="flex gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => copyToClipboard(firstLogWithSystemPrompt.systemPrompt)}
                      ...
                      <pre className="text-xs whitespace-pre-wrap break-words bg-amber-100 dark:bg-amber-800/30 p-2 rounded font-mono max-h-40 overflow-y-auto">
                        {firstLogWithSystemPrompt.systemPrompt}
                      </pre>
                    </div>
                  ) : null;
                })()}

                {/* Show all requests without repeating system prompt */}
                {logs
                  .filter(log => log.type === 'router_response')
                  .map((log) => (
                  <div
